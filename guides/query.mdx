---
title: 'Query'
description: 'Learn how to query game state from your World Engine shard using the Argus Labs SDK.'
---
...
The `where` field uses the [Expr expression language](https://expr-lang.org/), which provides:
...
```typescript Expression Examples
// Check if player name contains specific text
where: 'playertag.argus_auth_name contains "admin"'

// Multiple conditions
where: 'position.x > 100 && position.y < 200 && onlinestatus.online == true'

// Array membership
where: 'playertag.argus_auth_id in ["user1", "user2", "user3"]'

// Complex conditions with functions
where: 'len(playertag.argus_auth_name) > 5 && position.x + position.y < 500'
```
...
<Tip>If you're using React, you can use the [useQuery](/react/use-query) hook.</Tip>
