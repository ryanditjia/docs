...
```bash
export CARDINAL_SERVICE_ID="shard-1"
```
...
Disables command signature verification. This should only be used in development environments for testing purposes.

<Warning>
  Never disable signature verification in production environments as it compromises security.
</Warning>

```bash
export DISABLE_SIGNATURE_VERIFICATION="true"  # Development only
```
...
```bash
export PRIVATE_KEY="your-hex-encoded-ed25519-private-key"
```
